...
class ReviewApi {
  ...
  Future<Map<String, dynamic>> submitReview({
    ...
    if ((existing as List).isNotEmpty) {
      throw Exception('You have already submitted a review for this restaurant. Edit or delete your old one to post again.');
    }

    // 3. Clean and deduplicate dish mentions
    final uniqueDishes = dishMentions.map((d) => d.trim()).where((d) => d.isNotEmpty).toSet().toList();
  ...
  Future<void> rollbackDishMetricsPublic(String restaurantId, List<String> dishNames, double rating) async {
    ...
    for (final name in dishNames) {
      try {
        ...
        if (dish != null) {
          ...
          if (oldCount <= 1) {
            await _supabase.from('dishes').update({'mention_count': 0, 'trending_score': 0.0}).eq('id', dish['id']);
          } else {
            final int newCount = oldCount - 1;
            final double normalizedRating = reviewRating * 20.0;
  ...
  Future<void> voteReview(String reviewId) async {
    ...
    final existing = await _supabase.from('review_votes').select('id').eq('review_id', reviewId).eq('voter_id', voterId).maybeSingle();
    if (existing != null) throw Exception('You already voted.');

    await _supabase.from('review_votes').insert({'review_id': reviewId, 'voter_id': voterId});
    final review = await _supabase.from('reviews').select('helpful_votes, user_id').eq('id', reviewId).single();
    await _supabase.from('reviews').update({'helpful_votes': (review['helpful_votes'] as int) + 1}).eq('id', reviewId);
    await _supabase.rpc('increment_helpful_votes', params: {'target_user_id': review['user_id']});
    await TierUpgradeApi().checkUpgrade(review['user_id'] as String);
  }

  Future<void> unvoteReview(String reviewId) async {
    await _supabase.from('review_votes').delete().eq('review_id', reviewId).eq('voter_id', _currentUserId);
    final review = await _supabase.from('reviews').select('helpful_votes, user_id').eq('id', reviewId).single();
    await _supabase.from('reviews').update({'helpful_votes': (review['helpful_votes'] as int) > 0 ? (review['helpful_votes'] as int) - 1 : 0}).eq('id', reviewId);
    await _supabase.rpc('decrement_helpful_votes', params: {'target_user_id': review['user_id']});
  }

  Future<void> flagReview(String reviewId) async {
    await _supabase.from('reviews').update({'flagged': true}).eq('id', reviewId);
  }

  Future<List<Map<String, dynamic>>> getRestaurantReviews(String restaurantId, {int page = 1, int pageSize = 20}) async {
  ...
  Future<void> _recalculateScore(String restaurantId) async {
    final rawReviews = await _supabase.from('reviews').select('rating, users(tier)').eq('restaurant_id', restaurantId).eq('flagged', false);
    final reviewInputs = (rawReviews as List<dynamic>).map((r) => ReviewInput(tier: r['users']['tier'] as String, rating: (r['rating'] as num).toDouble())).toList();

    double popularity = 50.0; 
    try {
      final existing = await _supabase.from('algorithm_scores').select('popularity_score').eq('restaurant_id', restaurantId).maybeSingle();
      if (existing != null) popularity = (existing['popularity_score'] as num).toDouble();
    } catch (_) {}

    final result = RestaurantScoreCalculator.calculate(reviews: reviewInputs, popularity: popularity, proximity: 50.0);
    ...
    await _supabase.from('restaurants').update({'algorithm_score': result.finalScore, 'score_updated_at': DateTime.now().toIso8601String()}).eq('id', restaurantId);
  }
}
