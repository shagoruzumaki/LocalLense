...
class DiscoveryRepository {
  ...
  Future<List<RestaurantWithScore>> searchRestaurants(
    ...
    try {
      ...
      // 1. Search by name, category, or tags
      var nameQuery = _db.from('restaurants').select().eq('active', true);
      if (category != null && category != 'All') {
        nameQuery = nameQuery.eq('category', category.toLowerCase().replaceAll(' ', '_'));
      }
      
      final List byNameRes = await nameQuery.or('name.ilike.$pattern,address.ilike.$pattern');
      ...
      // 2. Search by dishes (optional - used as fallback if requested)
      if (searchByDish) {
        final List dishMatches = await _db.from('dishes').select('restaurant_id').ilike('name', pattern);
  ...
  Future<List<Dish>> searchDishes(String query, {int limit = 20}) async {
    ...
    try {
      ...
      // Fallback: simple search without join
      try {
        final List response = await _db.from('dishes')
            .select()
            .ilike('name', pattern)
