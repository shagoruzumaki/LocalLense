...
class DiscoveryRepository {
  ...
  Future<List<RestaurantWithScore>> getRestaurants(
    ...
    try {
      ...
      _sortRestaurantResults(result, filters.sortBy);
      return result;
    } catch (e) {
      rethrow;
    }
      ...
      if (userLat != null && userLng != null) {
        dishes.sort((a, b) {
          double distA = LocationUtils.calculateDistance(
            userLat,
            userLng,
            a.restaurantLatitude ?? 0,
            a.restaurantLongitude ?? 0,
          );
          double distB = LocationUtils.calculateDistance(
            userLat,
            userLng,
            b.restaurantLatitude ?? 0,
            b.restaurantLongitude ?? 0,
          );
  ...
  Future<List<RestaurantWithScore>> getHiddenGems({
    ...
    try {
      ...
      enriched = enriched
          .where((e) => (e.score?.reviewCount ?? 0) < 30)
          .toList();

      if (userLat != null && userLng != null) {
        enriched.sort(
          (a, b) => (a.distanceKm ?? 1000).compareTo(b.distanceKm ?? 1000),
        );
      }
  ...
  Future<List<RestaurantWithScore>> getOffersAndDeals({int limit = 10}) async {
    try {
      ...
      final restaurantIds = dishResponse
          .map((d) => d['restaurant_id'].toString())
          .toSet()
          .toList();

      if (restaurantIds.isEmpty) return [];

      final List resResponse = await _db
          .from('restaurants')
          .select()
          .inFilter('id', restaurantIds)
          .limit(limit);

      final restaurants = resResponse
          .map((json) => Restaurant.fromSupabase(json))
          .toList();
      final scores = await _fetchScores(restaurants.map((r) => r.id).toList());
  ...
  Future<List<RestaurantWithScore>> searchRestaurants(
    ...
    try {
      ...
      if (areaPart != null && cityPart != null) {
        ...
        if (response.isEmpty) {
          response = await _db
              ...
              .select()
              .eq('active', true)
              .or('address.ilike.%$areaPart%,address.ilike.%$cityPart%');
        }
      } else {
      ...
      if (searchByDish && areaPart == null) {
        final List dishMatches = await _db
            .from('dishes')
            .select('restaurant_id')
            .ilike('name', '%$q%');
        final dishRestaurantIds = dishMatches
            .map((d) => d['restaurant_id'].toString())
            .toSet();
        if (dishRestaurantIds.isNotEmpty) {
          final List byDishRes = await _db
              ...
              .select()
              .eq('active', true)
              .inFilter('id', dishRestaurantIds.toList());
      ...
      _sortRestaurantResults(result, sortBy);
      return result.take(limit).toList();
    } catch (e) {
      return [];
    }
  ...
  Future<List<Dish>> searchDishes(
    ...
    try {
      List response;
      if (areaPart != null) {
        final List restaurantsResponse = await _db
            .from('restaurants')
            .select('id')
            .ilike('address', '%$areaPart%');
        final restaurantIds = restaurantsResponse
            .map((r) => r['id'].toString())
            .toList();
        if (restaurantIds.isEmpty) return [];
      ...
      final List<Dish> dishes = response
          .map((json) => Dish.fromSupabase(json))
          .toList();
      ...
      for (var json in response) {
        suggestions.add(
          SearchSuggestion(
            ...
            type: SuggestionType.restaurant,
  ...
  Future<List<RestaurantWithScore>> getRanked({
    double? userLat,
    double? userLng,
    int? limit,
  }) async {
  ...
  Future<List<RestaurantWithScore>> getNearby({
    ...
    try {
      ...
      final scores = await _fetchScores(restaurants.map((r) => r.id).toList());
      var result = _enrich(restaurants, scores, userLat, userLng)
          .where((item) => (item.distanceKm ?? double.maxFinite) <= radiusKm)
          .toList();
      if (openNow) result = result.where((r) => r.isOpenNow).toList();
      result.sort(
  ...
  Future<Map<String, AlgorithmScore>> _fetchScores(List<String> ids) async {
    ...
    return {
      for (var json in response)
        json['restaurant_id'].toString(): AlgorithmScore.fromSupabase(json),
    };
  }

  List<RestaurantWithScore> _enrich(
    List<Restaurant> restaurants,
    Map<String, AlgorithmScore> scores,
    double? userLat,
    double? userLng,
  ) {
  ...
  void _sortRestaurantResults(
    List<RestaurantWithScore> result,
    SortOption sortBy,
  ) {
    switch (sortBy) {
      case SortOption.nearest:
        result.sort((a, b) {
          final distanceCompare = (a.distanceKm ?? double.maxFinite).compareTo(
            b.distanceKm ?? double.maxFinite,
          );
          if (distanceCompare != 0) return distanceCompare;
          return _compareByBestScore(a, b);
        });
        break;
      case SortOption.budget:
        result.sort((a, b) {
          final priceCompare = a.restaurant.priceTier.compareTo(
            b.restaurant.priceTier,
          );
      ...
      case SortOption.popular:
        result.sort(_compareByBestScore);
        break;
    }
  }

  int _compareByBestScore(RestaurantWithScore a, RestaurantWithScore b) {
    final scoreCompare = (b.restaurant.algorithmScore ?? 0.0).compareTo(
      a.restaurant.algorithmScore ?? 0.0,
    );
